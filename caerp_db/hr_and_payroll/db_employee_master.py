...
def save_employee_master_new(db: Session, request: EmployeeDetailsNew, employee_id: int, id:  List[int], user_id: int, Action: RecordActionType, employee_profile_component: Optional[str] = None):
  try:
    if (employee_id > 0 or employee_id < 0) and Action == RecordActionType.INSERT_ONLY:
      raise HTTPException(status_code=400, detail="ID should be 0 for inserting new employee master")
