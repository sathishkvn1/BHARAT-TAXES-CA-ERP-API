...
from caerp_db.common.models import Employee,OtpGeneration
...
def get_token(no_of_attempts: int =0 , request_data: OAuth2PasswordRequestForm = Depends(), user_agent: str = Header(None), request: Request = None, db: Session = Depends(get_db)):
    ...
    if not Hash.verify(hashed_password_from_db, plain_text_password_from_request):
        login_attempt = 3 - no_of_attempts
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail=f'{login_attempt} out of 3 attempts remaining.' ,  headers={"X-Error": "Invalid password."})
        # raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,message=f'{login_attempt} out of 3 attempts remaining.' , detail='Invalid password')
    ...
    try:
        ...
        print("employee Data",Employee.employee_id)
